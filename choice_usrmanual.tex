...
通常情况下使用\tn{choice}命令就够了, 但是用户如果有排版图片的需求, 可能需要将\meta{label}置于内容的上方或下方, 而\tn{choice}命令基于\env{hlist}环境编写, 所以\meta{label}只能置于内容的左侧, 这个时候只需要使用\tn{choice*}, 并在\oarg{键值列表}中更改\meta{anchor}为想要的效果即可. 
