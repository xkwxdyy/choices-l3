...
\begin{documentation}
\section{宏包简介}
\pkg{choice}宏包是一个用于排版选项的宏包, 包含但不限于以下特点：
\begin{enumerate}
	\item 可以排版任意数量的选项
	\item 可以方便切换标签风格\meta{label-style}
	\item 可以更改标签\meta{label}与内容的相对位置
	\item 可以调整标签\meta{label}的偏移
	\item 自动识别是否使用 \tn{includegraphics} 命令并自行调整 \cmd{anchor} 的位置（仅 \tn{choice*} 有此功能,  之所以会有 \tn{choice*} 命令, 就是因为基于 \env{hlist} 环境的 \tn{choice} 只有一个方位, 无法更改, 所以需要用另外的方法处理（ \tn{choice*} 是用 \cmd{coffin} 进行处理））.
	\emph{
		此功能的实现需要将 \pkg{expl3} 宏包更新至最新（至少是2021-11-12后）, 否则可能无法使用且会报错.
	}
\end{enumerate}

在需要排版选项的情况中（比如试卷、问卷排版等）\pkg{choice}宏包可以发挥重要作用, 旨在让用户更多关注在内容本身, 契合\LaTeX{}的内容与样式分离的思想. 

\pkg{choice}宏包是基于\env{hlist}环境与\LaTeX3开发的\LaTeX 宏包, 它提供了\tn{choice(*)} 两个主命令与\tn{coffinchoice}、\tn{hlistchoice}和\tn{quan}三个副命令. 

\tn{choice}和\tn{choice*}是利用了\pkg{xparse}宏包对\tn{hlistchoice}与\tn{coffinchoice} 命令进行封装. 其中\tn{choice}等效于\tn{hlistchoice}, \tn{choice}等效于\tn{coffinchoice}. 

通常情况下使用 \tn{choice} 命令就够了, 但是用户如果有排版图片的需求, 可能需要将 \meta{label} 置于内容的上方或下方, 而 \tn{choice} 命令基于 \env{hlist} 环境编写, 所以 \meta{label} 只能置于内容的左侧, 这个时候只需要使用\tn{choice*}, 会自动更改 \cmd{anchor} 为 \cmd{south}, 如果需要修改自动的 \cmd{anchor} 为其它的方位可以使用
\begin{LaTeXdemo}
	\choicesetup{
		autopic = north
	}
...
\emph{ 请将 \pkg{expl3} 宏包更新至最新（至少是2021-11-12后）, 否则可能无法使用 \pkg{choice} 宏包 }
\subsection{主要命令}

\begin{function}[added = 2021-12-18,updated = 2021-12-22]{\choice, \choice*}
	\begin{syntax}
		|\choice| \oarg{键值列表} \marg{内容}
		|\choice*| \oarg{键值列表} \marg{内容}
	\end{syntax}
...
\begin{function}[added = 2021-12-23]{\choicesetup}
	...
	\end{syntax}
	宏包相关键值的设置, 影响使用该命令后面的\tn{choice(*)}命令的相关初始值. 详细键值说明见 \ref{subsec:键值说明}.
\end{function}
...
\begin{function}[added = 2021-12-18, updated = 2021-12-22]{\hlistchoice}
	...
	\end{syntax}
	基于 \env{hlist} 环境写的选项排版命令, 可直接使用, 等效于\tn{choice}.
\end{function}
...
\begin{function}[added = 2021-12-21, updated = 2021-12-22]{\coffinchoice}
	...
	\end{syntax}
	基于 \LaTeX3 的 \pkg{coffin} 模块写的选项排版命令, 可直接使用, 等效于\tn{choice*}.
\end{function}
...
\subsection{\pkg{choice}宏包相关的键值说明}\label{subsec:键值说明}
...
\subsubsection{\tn{choice}与\tn{choice*}作用均有效的键值}
\begin{function}{label-style}
	\begin{syntax}
		label-style = \meta{arabic, alph, Alph, roman, Roman, quan} \init{Alph}
	\end{syntax}
	设置标签的风格: 
	\begin{itemize}
		\item arabic: 阿拉伯数字
...
\begin{function}{prelabel}
	\begin{syntax}
		|prelabel| = \marg{sth placed before label} \init{{}}
	\end{syntax}
...
\begin{function}[updated = 2021-12-24]{poslabel, postlabel}
	\begin{syntax}
		|poslabel| = \marg{sth placed after label} \init{{.}}
		|postlabel| = \marg{sth placed after label} \init{{.}}
	\end{syntax}
...
\subsubsection{仅对\tn{choice*}产生效果的键值}
\remark{
	下面所说的键值如果作用在\tn{choice}上并不会报错, 但不会产生作用, 这么设计是为了方便用户在\tn{choice}与\tn{choice*}两个命令之间自由切换而不用考虑这个键值.
}
\begin{function}[updated = 2021-12-23]{anchor}
	...
	\begin{itemize}
		\item north: 标签位于内容的\emph{正上方}
		\item south: 标签位于内容的\emph{正下方}
		\item east: 标签位于内容的\emph{右边}
		\item west: 标签位于内容的\emph{左边}
		\item northwest: 标签位于内容的\emph{左上角}
		\item northeast: 标签位于内容的\emph{右上角}
    \item southwest: 标签位于内容的\emph{左下角}
		\item southeast: 标签位于内容的\emph{右下角}
	\end{itemize}
\end{function}
...
\begin{function}[added = 2021-12-25]{autopic, autopic-anchor}
	\begin{syntax}
		|autopic| = \meta{方位}  \init{south}
		|autopic-anchor| = \meta{方位} \init{south}
	\end{syntax}
	\pkg{choice} 宏包设置了检测是否内容中包含 \tn{includegraphics} 命令并自动进行 \kvopt{\meta{anchor}}{\meta{方位}} 的设置, 默认是\kvopt{\meta{anchor}}{\meta{south}}, 更改 \cmd{autopic} 的值可更改默认方位.
...
\begin{function}[added = 2021-12-25]{belowsep, itembelowsep}
	\begin{syntax}
		|belowsep| = \meta{dimension}
		|itembelowsep| = \meta{dimension}
	\end{syntax}
...
\begin{function}{colsep, col-sep, columnsep, column-sep, itemsep, item-sep}
	\begin{syntax}
		|colsep| = \meta{dimension}
		|col-sep| = \meta{dimension}
		|columnsep| = \meta{dimension}
		|column-sep| = \meta{dimension}
		|itemsep| = \meta{dimension}
...
% \choice*[label-style = alph, bottom = 1em]{
% 	\includegraphics[width = 2cm]{example-image-a} &&
% 	\includegraphics[width = 2cm]{example-image-a} &&
% 	\includegraphics[width = 2cm]{example-image-a} &&
% 	\includegraphics[width = 2cm]{example-image-a} &&
...
\begin{LaTeXautodemo}
	\choice*[anchor = south, label-style = roman]{
		\includegraphics[width = 2cm]{example-image-a} &&
		\includegraphics[width = 2cm]{example-image-a} &&
		\includegraphics[width = 2cm]{example-image-a} &&
		\includegraphics[width = 2cm]{example-image-a} &&
...
% \choice*[anchor = south, label-style = roman, bottom = 0.5em]{
% 	\includegraphics[width = 2cm]{example-image-a} &&
% 	\includegraphics[width = 2cm]{example-image-a} &&
% 	\includegraphics[width = 2cm]{example-image-a} &&
% 	\includegraphics[width = 2cm]{example-image-a} &&
...
\begin{LaTeXautodemo}
	\choice*[anchor = north, label-style = Roman]{
		\includegraphics[width = 2cm]{example-image-a} &&
		\includegraphics[width = 2cm]{example-image-a} &&
		\includegraphics[width = 2cm]{example-image-a} &&
		\includegraphics[width = 2cm]{example-image-a} &&
...
% \choice*[anchor = north, label-style = Roman, bottom = 0.5em]{
% 	\includegraphics[width = 2cm]{example-image-a} &&
% 	\includegraphics[width = 2cm]{example-image-a} &&
% 	\includegraphics[width = 2cm]{example-image-a} &&
% 	\includegraphics[width = 2cm]{example-image-a} &&
...
\begin{LaTeXautodemo}
	\choice*[anchor = east, label-style = quan, colsep = -2em]{
		\includegraphics[width = 2cm]{example-image-a} &&
		\includegraphics[width = 2cm]{example-image-a} &&
		\includegraphics[width = 2cm]{example-image-a} &&
		\includegraphics[width = 2cm]{example-image-a} &&
